...
export function EvidenceTable({ items }: { items: Row[] }) {
  ...
  const canEdit = useCan("evidence:edit");

  useEffect(() => {
    if (open) {
      fetch("/api/requirements")
        .then((r) => r.json())
        .then((j) => setReqs(j.requirements));
      fetch("/api/documents")
        .then((r) => r.json())
  ...
  const filtered = useMemo(
    () =>
      items.filter(
        (e) =>
          (!q ||
            `${e.title} ${e.notes}`.toLowerCase().includes(q.toLowerCase())) &&
          (!type || e.evidence_type === type) &&
      ...
      <div className="card mb-4">
        <div className="card-body grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          >
            ...
            {["Valid", "Expired", "Pending Update", "Rejected"].map((t) => (
              <option key={t}>{t}</option>
            ))}
          </select>
          {canCreate && (
            ...
            {filtered.map((e) => (
              <tr key={e.id}>
                <td className="wrap max-w-xs">
                  ...
                  <div className="text-xs text-slate-500">{e.notes}</div>
                </td>
                ...
                <td className="text-xs space-x-2">
                  {canEdit ? (
                    <>
                      ...
                      {e.status !== "Expired" && (
                        <button
                          onClick={() => markStatus(e.id, "Expired")}
                          className="text-red-700 hover:underline"
                        >
                          Mark Expired
                        </button>
                      )}
                      {e.status !== "Pending Update" && (
                        <button
                          onClick={() => markStatus(e.id, "Pending Update")}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setData({ ...data, requirement_id: e.target.value || null })
              }
            >
              <option value="">—</option>
              {reqs.map((r) => (
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setData({ ...data, document_id: e.target.value || null })
              }
            >
              <option value="">—</option>
              {docs.map((d) => (
