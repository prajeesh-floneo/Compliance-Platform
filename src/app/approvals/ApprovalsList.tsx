...
export function ApprovalsList({ items }: { items: Row[] }) {
  ...
  const canSign = useCan("approvals:sign");
  const [tab, setTab] = useState<"all" | "pending" | "approved" | "rejected">(
    "pending",
  );
  const [active, setActive] = useState<Row | null>(null);
  const [mode, setMode] = useState<
    "approve" | "reject" | "changes" | "comment" | "sign" | null
  >(null);
    ...
    () =>
      items.filter((a) =>
        tab === "all"
          ? true
          : tab === "pending"
            ? a.status === "Pending" || a.status === "Changes Requested"
            : tab === "approved"
              ? a.status === "Approved"
              : a.status === "Rejected",
      ),
  ...
  const doAction = async () => {
    ...
    await fetch("/api/approvals", {
      ...
      body: JSON.stringify({ id: active.id, action: mode, comments }),
    });
    setActive(null);
    setMode(null);
    setComments("");
  ...
  const tabs = [
    {
      key: "pending",
      label: `Pending (${items.filter((i) => i.status === "Pending" || i.status === "Changes Requested").length})`,
    },
    {
      key: "approved",
      label: `Approved (${items.filter((i) => i.status === "Approved").length})`,
    },
    {
      key: "rejected",
      label: `Rejected (${items.filter((i) => i.status === "Rejected").length})`,
    },
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((a) => (
                <tr key={a.id}>
                  ...
                  <td className="wrap max-w-md text-slate-600 text-xs">
                    {a.comments}
                  </td>
                  <td className="text-xs space-y-1">
                    {canDecide ? (
                      <>
                        <button
                          className="block text-emerald-700 hover:underline flex items-center gap-1"
                          onClick={() => {
                            setActive(a);
                            setMode("approve");
                            setComments("");
                        ...
                        <button
                          className="block text-red-700 hover:underline flex items-center gap-1"
                          onClick={() => {
                            setActive(a);
                            setMode("reject");
                            setComments("");
                        ...
                        <button
                          className="block text-amber-700 hover:underline flex items-center gap-1"
                          onClick={() => {
                            setActive(a);
                            setMode("changes");
                            setComments("");
                    ...
                    {canSign &&
                      a.item_type === "Document" &&
                      a.status !== "Approved" && (
                        <button
                          className="block text-brand-700 hover:underline flex items-center gap-1"
                          onClick={() => {
                            setActive(a);
                            setMode("sign");
                            setComments("");
                        ...
                        >
                          <PenLine className="h-3 w-3" /> Sign Document
                        </button>
                      )}
                  </td>
                </tr>
        ...
        footer={
          <>
            ...
            <button className="btn-primary" onClick={doAction}>
              {mode === "sign" ? "Sign & Approve" : "Confirm"}
            </button>
          </>
        }
      >
        <p className="text-sm text-slate-600 mb-3">
          <strong>{active?.item_type}:</strong> {active?.item_name}
        </p>
        {mode === "sign" && (
          <p className="text-xs text-slate-500 mb-2">
            Records signer name, role, timestamp and signature hash on the
            document. Placeholder for integration with WebSec / e.firma.
          </p>
        )}
